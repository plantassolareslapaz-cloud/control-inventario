...
def init_db():
    # Nos conectamos a Neon en vez de al archivo local
...
def registrar_movimiento(movimiento: Movimiento):
    with psycopg2.connect(DB_URL) as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute("UPDATE productos SET existencia = %s WHERE codigo = %s", (nuevo_stock, movimiento.id))
            conn.commit()
...
def editar_producto(codigo: str, datos: UpdateProducto):
    if datos.password != ADMIN_PASSWORD:
        raise HTTPException(status_code=401, detail="Contraseña incorrecta. No tienes permisos.")
        ...
        with conn.cursor() as cursor:
            cursor.execute("""
                UPDATE productos SET 
                    ...
                    unidad = %s, ubicacion = %s, ultimo_costo = %s, moneda = %s, precio_mayoreo = %s, precio_publico = %s
                WHERE codigo = %s
            """, (datos.familia, datos.marca, datos.descripcion, datos.estatus, datos.existencia,
                  datos.unidad, datos.ubicacion, datos.ultimo_costo, datos.moneda, datos.precio_mayoreo, datos.precio_publico, codigo))
            conn.commit()
